...
export class QuotesComponent implements OnInit {
  ...
  LoadQuotes() {
    ...
    this.service.GetQuotes().subscribe((res) => {
      if (res.status == true) {
        let quotes = res.quotesLists;
        quotes.forEach((quotes, i) => (quotes["sNo"] = i + 1));
        this.quotesList = quotes;
      }
      console.log("Quotes", this.quotesList);
    });
  ...
  isLastPage(): boolean {
    return this.quotesList
      ? this.first === this.quotesList.length - this.rows
      : true;
  }
  isFirstPage(): boolean {
    return this.quotesList ? this.first === 0 : true;
  }
